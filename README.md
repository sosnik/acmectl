...
acme-hooked, to register an account for you and sign all following requests.
...
[JWK](https://tools.ietf.org/html/rfc7517) format. `acme-hooked` is using the PEM
...
python acme_hooked.py --account-key ./account.key --csr ./domain.csr --acme-dir /var/www/challenges/ > ./signed_chain.crt
```
...
```sh
#!/usr/bin/sh
python /path/to/acme_hooked.py --account-key /path/to/account.key --csr /path/to/domain.csr --acme-dir /var/www/challenges/ > /path/to/signed_chain.crt.tmp || exit
...
NOTE: Since Let's Encrypt's ACME v2 release (acme-hooked 4.0.0+), the intermediate
...
signed certificate. If you have an bash script using acme-hooked &lt;4.0 (e.g. before
2018-03-17) with acme-hooked 4.0.0+, then you may be adding the intermediate
...
it to the acme-hooked certificate output.
